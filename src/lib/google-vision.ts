...
function getClient() {
  if (!client) {
    ...
    if (!credsEnv) {
      console.warn(
        '[Google Vision] GOOGLE_APPLICATION_CREDENTIALS not configured - Vision API disabled'
      );
      return null;
    }
...
export async function checkForPeopleAndFaces(imageBuffer: Buffer) {
  try {
    ...
    const personObjects = objects.filter((obj) => obj.name?.toLowerCase() === 'person');
...
export async function generatePromptFromImage(
  ...
  try {
    ...
    const isNature = topLabels.some(
      (l) =>
        l.toLowerCase().includes('nature') ||
        l.toLowerCase().includes('landscape') ||
        l.toLowerCase().includes('sky') ||
        l.toLowerCase().includes('mountain') ||
        l.toLowerCase().includes('tree') ||
        l.toLowerCase().includes('water')
    );

    const isUrban = topLabels.some(
      (l) =>
        l.toLowerCase().includes('building') ||
        l.toLowerCase().includes('city') ||
        l.toLowerCase().includes('street') ||
        l.toLowerCase().includes('architecture')
    );

    const isIndoor = topLabels.some(
      (l) =>
        l.toLowerCase().includes('room') ||
        l.toLowerCase().includes('indoor') ||
        l.toLowerCase().includes('furniture') ||
        l.toLowerCase().includes('interior')
    );
    ...
    const secondaryObjects = topObjects
      .slice(1, 4)
    ...
    if (language === 'zh') {
      structuredPrompt = `一个${mainSubject} ${action}，位于${environment}，在${lighting}${timeOfDay}的条件下，营造${mood}的氛围，采用${visualStyle}，${composition}的构图方式，展现${specificFocus}等视觉细节。${quality}${colorKeywords ? `，${colorKeywords}色调` : ''}`;
    } else if (language === 'ja') {
      structuredPrompt = `${mainSubject}が${action}で、${environment}に配置され、${timeOfDay}の${lighting}の下で、${mood}雰囲気を演出し、${visualStyle}で、${composition}で撮影され、${specificFocus}を表示します。${quality}${colorKeywords ? `、${colorKeywords}` : ''}`;
    } else if (language === 'fr') {
      structuredPrompt = `Un ${mainSubject} ${action}, dans ${environment}, sous ${lighting} pendant ${timeOfDay}, créant une atmosphère ${mood}, dans le style de ${visualStyle}, capturé avec ${composition}, montrant ${specificFocus}. ${quality}${colorKeywords ? `, ${colorKeywords}` : ''}`;
    } else if (language === 'es') {
      structuredPrompt = `Un ${mainSubject} ${action}, en ${environment}, bajo ${lighting} durante ${timeOfDay}, transmitiendo una atmósfera ${mood}, al estilo de ${visualStyle}, capturado con ${composition}, mostrando ${specificFocus}. ${quality}${colorKeywords ? `, ${colorKeywords}` : ''}`;
    } else {
      structuredPrompt = `A ${mainSubject} ${action}, in ${environment}, under ${lighting} during ${timeOfDay}, conveying a ${mood} atmosphere, in the style of ${visualStyle}, captured with ${composition}, showing ${specificFocus}. ${quality}${colorKeywords ? `, ${colorKeywords}` : ''}`;
    }

...
export async function POST(request: NextRequest) {
  try {
    ...
    if (activeSubscription) {
      ...
      console.log(
        `[Create Checkout] User ${session.user.id} has active ${currentPlan} ${currentInterval} subscription`
      );
      ...
      if (!activeSubscription.cancelAtPeriodEnd) {
        console.log(
          `[Create Checkout] Auto-canceling existing ${currentPlan} subscription for plan change`
        );

        try {
          if (!activeSubscription.subscriptionId) {
            console.log(
              `[Create Checkout] No subscription ID found for subscription ${activeSubscription.id}`
            );
          ...
          if (cancelResult.success) {
            ...
            console.log(
              `[Create Checkout] Successfully canceled ${currentPlan} subscription, proceeding with ${planId} checkout`
            );
          } else {
        ...
        console.log(
          '[Create Checkout] Existing subscription already set to cancel, proceeding with new checkout'
        );
      }
    }

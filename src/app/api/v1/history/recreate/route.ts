...
export async function POST(req: NextRequest) {
  try {
    ...
    if (history.status === 'expired') {
      return NextResponse.json({ error: 'Cannot recreate expired content' }, { status: 400 });
    }
    ...
    return NextResponse.json({
      ...
      message:
        'You can now recreate this content. Use the original endpoint with these parameters.',
    });
  } catch (error) {
    ...
    return NextResponse.json({ error: 'Failed to recreate from history' }, { status: 500 });
  }
}

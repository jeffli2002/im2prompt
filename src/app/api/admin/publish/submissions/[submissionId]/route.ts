...
async function awardPublishReward(submission: PublishSubmission) {
  const reward = SHARE_REWARD_CONFIG.publishIm2prompt ?? SHARE_REWARD_CONFIG.publishViecom;
  ...
  try {
    await creditService.earnCredits({
      ...
      source: 'social_share',
      description: 'Publish on im2prompt.com (admin approved)',
      referenceId: `social_share_${shareId}`,
      metadata: {
        submissionId: submission.id,
        assetType: submission.assetType,
        rewardType: reward.rewardType ?? 'publishIm2prompt',
      },
    });
  } catch (error) {

...
import { user, userCredits } from '@/server/db/schema';
...
export async function GET(request: Request) {
  try {
    ...
    // Build query (without subscription table, which may not exist)
    let query = db
      .select({
        id: user.id,
        email: user.email,
        ...
        // Fallback values for plan and subscriptionStatus
        // to keep response shape compatible with the UI
        plan: sql<string>`'free'`,
        subscriptionStatus: sql<string>`'inactive'`,
      ...
      .from(user)
      .leftJoin(userCredits, sql`${user.id} = ${userCredits.userId}`)

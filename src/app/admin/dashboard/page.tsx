  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        ...
        console.error('Non-JSON response from dashboard API:', text.substring(0, 200));
      ...
      if (response.ok) {
        ...
        // Don't throw, just log and set empty stats
        setStats(null);
      }
    } catch (error) {
      console.error('Failed to fetch stats:', error);
      // Set a minimal error state so the page doesn't hang
      setStats(null);
    } finally {
      setIsLoading(false);
    }

...
async function testFaceDetection(scenario) {
  ...
  try {
    ...
    if (apiResponse.status === 400 && responseData.error) {
      if (
        responseData.error.includes('face') ||
        responseData.error.includes('people') ||
        responseData.error.includes('person') ||
        responseData.error.includes('人脸')
      ) {
        console.log(
          `   ✅ CORRECT: Image was blocked by Vision API (${scenario.expectedResult === 'BLOCKED' ? 'Expected' : 'Unexpected'})`
        );
      ...
      console.log(
        `   ✅ CORRECT: Image passed Vision API check (${scenario.expectedResult === 'PASS' ? 'Expected' : 'Unexpected'})`
      );
